...
func TestOptions(t *testing.T) {
	handlers := map[string]TestHandler{
		...
		"with-v3-comments":             runWithV3CommentsTest,
...
// runWithV3CommentsTest tests WithV3Comments
func runWithV3CommentsTest(t *testing.T, tc TestCase) {
	t.Helper()

	args := parseBoolSlice(t, tc.From)
	opt := WithV3Comments(args...)
	opts := &Options{}
	err := opt(opts)
	...
	if tc.Like != "" {
		...
		assert.NoErrorf(t, err, "WithV3Comments error: %v", err)
		checkWantFields(t, opts, tc.Want)
	}
}
...
func checkWantFields(t *testing.T, opts *Options, want any) {
	...
	for key, expectedValue := range wantMap {
		...
		case "v3_comments":
			expected, ok := expectedValue.(bool)
			if !ok {
				t.Fatalf("want.v3_comments should be bool, got %T", expectedValue)
			}
			assert.Equalf(t, expected, opts.V3Comments, "V3Comments = %v, want %v", opts.V3Comments, expected)

		case "skip_comments":
			expected, ok := expectedValue.(bool)
			if !ok {
				t.Fatalf("want.skip_comments should be bool, got %T", expectedValue)
			}
			assert.Equalf(t, expected, opts.SkipComments, "SkipComments = %v, want %v", opts.SkipComments, expected)
