...
func TestScanner(t *testing.T) {
	RunTestCases(t, "scanner_test.yaml", map[string]TestHandler{
...
func runScanTokensTest(t *testing.T, tc TestCase) {
	...
	for _, item := range wantSlice {
		...
		expected = append(expected, ParseTokenType(tokenStr))
	}
...
func runScanTokensDetailedTest(t *testing.T, tc TestCase) {
	...
	for i, wantSpec := range tc.WantTokens {
		token := tokens[i]
		wantType := ParseTokenType(wantSpec.Type)
		...
		if wantSpec.Style != "" {
			wantStyle := ParseScalarStyle(wantSpec.Style)
