...
//
//	// Strip comments during loading for faster processing
//	loader, _ := yaml.NewLoader(r, yaml.WithPlugins(none.New()))
//
//	// Apply multiple plugins (they execute in order)
//	dumper, _ := yaml.NewDumper(w, yaml.WithPlugins(plugin1, plugin2))
//
// # Plugin Execution
//
...
//
//	// Usage
//	dumper, _ := yaml.NewDumper(w, yaml.WithPlugins(&UppercasePlugin{}))
//
// # Built-in Plugins
//
// The plugin/comment subpackage provides comment handling plugins:
//
//   - comment/v3 - v3-style comment preservation
//   - comment/v4 - v4-style comment preservation (auto-included with v4 options)
//   - comment/none - Strip all comments for performance
//
