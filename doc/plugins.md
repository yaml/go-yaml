...
## Quick Example: Stripping Comments

The simplest plugin use case is stripping comments for performance:
...
// Load YAML without comments (faster)
loader, _ := yaml.NewLoader(reader,
    yaml.WithPlugins(none.New()),
)
```

## Built-in Comment Plugins

go-yaml includes three comment handling plugins:
...
loader, _ := yaml.NewLoader(reader,
    yaml.WithPlugins(commentv3.New()),
)
```

### `plugin/comment/v4` - v4 Comment Handling

Enhanced comment handling for v4:

```go
import commentv4 "go.yaml.in/yaml/v4/plugin/comment/v4"

loader, _ := yaml.NewLoader(reader,
    yaml.WithPlugins(commentv4.New()),
)
```

**Combine with v4 preset:**
...
### `plugin/comment/none` - Strip Comments

Removes all comments for performance:
...
- You want faster parsing and less memory usage
- You're generating YAML programmatically
...
**Performance:** Use `plugin/comment/none` if you don't need comments.
...
**✓ Strip comments** - Use `plugin/comment/none`
