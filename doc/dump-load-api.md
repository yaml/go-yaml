# YAML Encoding and Decoding API

This guide covers go-yaml's encoding (dumping) and decoding (loading) APIs,
...
as of v4 and will be removed in v5. Use `Dump`/`Load` and `Dumper`/`Loader`
instead.
...
provide a full Dump and Load stack. Those involve the Representation (for Dump)
and Construct (for Load) YAML stack stages and v4 will offer those (optionally)
for Dump and Load.
...
go-yaml provides four levels of API complexity:

| Level | Encoding | Decoding | Use Case |
|-------|----------|----------|----------|
| Simple (DEPRECATED) | `Marshal` | `Unmarshal` | Quick conversions, no options needed |
| Simple Streaming (DEPRECATED) | `Encode` | `Decode` | Simple multi-doc streams, no options |
| Flexible | `Dump`, `DumpAll` | `Load`, `LoadAll` | Single/multi-doc with options |
| Flexible Streaming | `NewDumper` | `NewLoader` | Large files, continuous streams with options |

## DEPRECATED: Simple API (Marshal and Unmarshal)
...
## DEPRECATED: Simple Streaming API (Encoder and Decoder)
...
## New v4 Flexible API: Dump, DumpAll, Load, LoadAll
...
Encode a single value with v4 defaults (2-space indent, compact):
...
Encode multiple values as a multi-document stream:
...
Decode the first document with options:
...
Decode all documents from a multi-document stream:
...
## Flexible Streaming API: NewDumper and NewLoader

Full control over encoding/decoding with streaming support and configurable
options.
...
### Node.Dump() - Encode with Options

The `Node.Dump()` method lets you apply encoding options when building Node
...
- Preserving options through the full decode/encode pipeline

**Comparison with Decode/Encode:**

| Method | Options Support | Use Case |
|--------|----------------|----------|
| `node.Decode()` | ❌ No | Simple decoding, no options needed |
| `node.Load()` | ✅ Yes | Decoding with options (e.g., strict validation) |
| `node.Encode()` | ❌ No | Simple encoding, no options needed |
| `node.Dump()` | ✅ Yes | Encoding with options (e.g., custom indent) |
...
└─────────────────────────────────────────────────────────────┘
         ...
         └─────────────────┘   └─────────────────┘
             ...
             No      Yes            No      Yes
              ...
              ▼       ▼              ▼       ▼
         ┌─────┐ ┌─────────┐   ┌────────┐ ┌──────────┐
         │M/UM │ │Dump/    │   │Encode/ │ │NewDumper/│
         │     │ │Load/    │   │Decode  │ │NewLoader │
         │     │ │DumpAll/ │   │        │ │          │
         │     │ │LoadAll  │   │        │ │          │
         └─────┘ └─────────┘   └────────┘ └──────────┘

Legend: M/UM = Marshal/Unmarshal
```
